...
namespace DS.SPGenerator
{
	public partial class spGenMain : Form
	{
        ...
        private string dataaccessclassName = string.Empty;
		...
		{
			InitializeComponent();
            treeDefaultSPOptions.ExpandAll();
		}
		private void spGenMain_Load(object sender, EventArgs e)
		{
			...
			lblMsgUpdate.Text = "";
			lblMsgSPTypes.Text = "";
            lblMsgSPTypesDetails.Text = "";
            lblMsgDefaults.Text = "";
			lblMsgInsert.Text = "";
			lblMsgDelete.Text = "";
            lblMsgDefaultsDetails.Text = "";
            lblMsgInsertDetails.Text = "";
            lblMsgDeleteDetails.Text = "";
        ...
        private void cmdDefaultCopy_Click(object sender, EventArgs e)
        {
            bool bRes = CopyToClipboard(this.rtbDefaultSPs.Text);
        }
		...
		private void tmrMsg_Tick(object sender, EventArgs e)
		{
            ...
            this.lblMsgDefaults.Text = string.Empty;
            tmrMsg.Enabled = false;
		}
        private void tmrMsgDetails_Tick(object sender, EventArgs e)
        {
            this.lblMsgDeleteDetails.Text = string.Empty;
            this.lblMsgSPTypesDetails.Text = string.Empty;
            this.lblMsgInsertDetails.Text = string.Empty;
            this.lblMsgUpdateDetails.Text = string.Empty;
            this.lblMsgDefaultsDetails.Text = string.Empty;
            tmrMsgDetails.Enabled = false;
        }
        ...
        private void cmdDefaultPublish_Click(object sender, EventArgs e)
        {
            List<string> procs = new List<string>();
            procs.AddRange(BuildDefaultSelects());
            procs.AddRange(BuildDefaultInserts());
            procs.Add(BuildDefaultUpdates());
            procs.Add(BuildDefaultDeletes());
            int successcount = 0;
            int totalCount = procs.Count;
            foreach (string s in procs)
            {
                if (SqlHelper.PublishText(s, dbP.ConnectionString)) 
                {
                    successcount++;
                } 
            }
            DisplayMsg(successcount.ToString() + " of " + totalCount.ToString() + " Default Stored Procedures Published successfully", 4);
        }
		...
		private void cboSP_SelectedIndexChanged(object sender, EventArgs e)
		{
            ...
            dataaccessclassName = string.Empty;
            dataclassName = string.Empty;
		}
				...
				try
				{
					if (cboServers.Text.Length > 0)
					{
						if (CheckAuthSet())
						{
                            ...
                            if (dbList == null || dbList.Count() < 1 || (dbList.Count()==1 && dbList[0]==""))
                            {
                                dbP.DatabaseName = "Master";
                                dbList = sqlhlpr.EnumerateSQLServerDatabasesDirect(this.dbP.ConnectionString + "Connection Timeout=8;");
                            }
		...
		private void opEQUALTO_Click(object sender, EventArgs e)
		{
			...
			switch (tabAll.SelectedTab.Name.ToString())
			{
				case "tabSPTypes":
					this.rtbSelectSP.Rtf = sh.Highlight(BuildSelectSP());
					break;
		...
		private void DisplayMsg(string msg, int seconds)
		{
            ...
            this.lblMsgDefaults.Text = msg;
			this.tmrMsg.Interval = seconds * 1000;
			this.tmrMsg.Enabled = true;

		}
        private void DisplayMsgDetails(string msg, int seconds)
        {
            this.tmrMsgDetails.Enabled = false;
            this.lblMsgSPTypesDetails.Text = msg;
            this.lblMsgDeleteDetails.Text = msg;
            this.lblMsgInsertDetails.Text = msg;
            this.lblMsgUpdateDetails.Text = msg;
            this.lblMsgDefaultsDetails.Text = msg;
            this.tmrMsgDetails.Interval = seconds * 1000;
            this.tmrMsgDetails.Enabled = true;
        ...
        private string BuildDefaultProcedures()
        {
            StringBuilder sps = new StringBuilder();
            List<string> selects = BuildDefaultSelects();
            foreach (string s in selects) { sps.AppendLine(s); }
            //sps.Append(BuildDefaultSelects());
            sps.AppendLine();
            List<string> inserts = BuildDefaultInserts();
            foreach (string s in inserts) { sps.AppendLine(s); }
            //sps.Append(BuildDefaultInserts());
            sps.AppendLine();
            sps.Append(BuildDefaultUpdates());
            sps.AppendLine();
            sps.Append(BuildDefaultDeletes());
            return sps.ToString();
        }
        private TreeNode GetNode(string name)
        {
            try
            {
                return treeDefaultSPOptions.Nodes.Find(name,true)[0];
            }
            catch (Exception ex) { }
            return null;
        }
        private List<string> BuildDefaultSelects()
        {
            List<string> selects = new List<string>();
            if (dbP.IsValid() && !string.IsNullOrEmpty(dbP.Table))
            {
                selects.Add(BuildDefaultSelectAll());
                selects.Add(BuildDefaultSelectPK());
                selects.AddRange(BuildDefaultSelectIX());
                selects.AddRange(BuildDefaultSelectFK());
            }
            return selects;
        }
        private string BuildDefaultSelectAll()
        {
            StringBuilder selects = new StringBuilder();
            TreeNode treeNode = GetNode("nodeSelectsEverything");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                if (dbP.IsValid() && !string.IsNullOrEmpty(dbP.Table))
                {
                    string sSel = string.Empty;
                    selects.AppendLine("CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_SelectAll]");
                    if (bLoading == false)
                    {
                        ...
                        if (dbP.Fields == null || dbP.Fields.Count == 0) { dbP.GetTableInfo(); }
                        if (dbP.PrimaryKeys == null || dbP.PrimaryKeys.Count == 0) { dbP.GetPrimaryKeys(); }
                        foreach (ColumnProperties cp in dbP.Fields)
                        {
                            sSel += nl + tab + tab + cp.Column + ", ";
                        }
                        ...
                        selects.AppendLine();
                        selects.AppendLine(" ; ");
                        //selects.AppendLine(" GO ");
                        //selects.AppendLine(" SET ANSI_NULLS ON " + nl + " GO " + nl + " SET QUOTED_IDENTIFIER ON " + nl + " GO ");
                    }
                }
            }
            return selects.ToString();
        }
        private string BuildDefaultSelectPK()
        {
            StringBuilder selects = new StringBuilder();
            TreeNode treeNode = GetNode("nodeSelectsPK");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                if (dbP.IsValid())
                {
                    ...
                    if (bLoading == false)
                    {
                        ...
                        foreach (ColumnProperties cp in dbP.Fields)
                        {
                            ...
                            if (cp.PriKey)
                            {
                                pkFieldName += cp.Column;
                                sWhere += nl + tab + tab + "(@" + cp.Column + " = " + cp.Column + ") AND";
                                sParams += nl + tab + "@" + cp.Column + " " + cp.ColumnDefinition() + " ,";
                            }
                        }
                        sSel = sSel.Remove(sSel.Length - 2, 2);
                        if (sWhere.Length > 0) { sWhere = nl + tab + "WHERE" + nl + tab + "(" + sWhere.Remove(sWhere.Length - 4, 4) + nl + tab + ")"; }
                        if (sParams.Length > 0) { sParams = nl + "(" + sParams.Remove(sParams.Length - 2, 2) + nl + ")"; }
                        sSel += nl + tab + "FROM [" + dbP.Schema + "].[" + ((dbP.Table.Length > 0) ? dbP.Table : dbP.View) + "]";
                        sSel += sWhere;
                        sSel += nl + "END";
                        selects.AppendLine("CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_SelectBy" + pkFieldName.ToUpper() + "]");
                        selects.AppendLine(sParams);
                        selects.Append(sSel);
                        selects.AppendLine();
                        selects.AppendLine(" ; ");
                        //selects.AppendLine(" GO ");
        ...
        private List<string> BuildDefaultSelectIX()
        {
            List<string> selCollection = new List<string>();
            StringBuilder selects = new StringBuilder();
            TreeNode treeNode = GetNode("nodeSelectsIndexes");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                if (dbP.IsValid())
                {
                    if (bLoading == false)
                    {
                        ...
                        if (dbP.Fields == null || dbP.Fields.Count == 0) { dbP.GetTableInfo(); }
                        if (dbP.Indices == null || dbP.Indices.Count == 0) { dbP.GetIndices(); }
                        foreach (KeyValuePair<string, dbIndex> ix in dbP.Indices)
                        {
                            // separate SP required foreach index in table
                            string sSel = string.Empty;
                            if (ix.Value.IndexColumns != null && ix.Value.IndexColumns.Count > 0)
                            {
                                string concatFieldList = string.Empty;
                                string sWhere = string.Empty;
                                string sParams = string.Empty;
                                foreach (string ixField in ix.Value.IndexColumns)
                                {
                                    concatFieldList += ixField;
                                    sParams += nl + tab + "@" + CP(ixField) + " " + ((ColumnProperties)dbP.Columns[ixField]).ColumnDefinition() + ", ";
                                    sWhere += nl + tab + tab + "([" + ixField + "] = @" + CP(ixField) + ") AND";
                                }
                                selects.AppendLine("CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_SelectBy" + concatFieldList.ToUpper() + "]");
                                ...
                                sSel += sWhere;
                                sSel += nl + "END" + nl;
                                selects.Append(sParams);
                                selects.Append(sSel);
                                //selects.AppendLine(" ; ");
                                selCollection.Add(selects.ToString());
                                selects.Clear();
                                //selects.AppendLine(" GO ");
                                //selects.AppendLine(" SET ANSI_NULLS ON " + nl + " GO " + nl + " SET QUOTED_IDENTIFIER ON " + nl + " GO ");
                                selects.AppendLine();
        ...
        private List<string> BuildDefaultSelectFK()
        {
            List<string> selCollection = new List<string>();
            List<ColumnProperties> fkCollection = new List<ColumnProperties>();
            string sSelList = string.Empty;
            TreeNode treeNode = GetNode("nodeSelectsFK");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                if (dbP.IsValid())
                {
                    if (bLoading == false)
                    {
                        foreach (ColumnProperties cp in dbP.Fields)
                        {
                            sSelList += nl + tab + tab + cp.Column + ", ";
                            if (cp.ForeignKeyProperties.IsFK)
                            {
                                fkCollection.Add(cp);
                            }
                        }
                        sSelList = sSelList.Remove(sSelList.Length - 2, 2);
                        if (dbP.Fields == null || dbP.Fields.Count == 0) { dbP.GetTableInfo(); }
                        if (dbP.PrimaryKeys == null || dbP.PrimaryKeys.Count == 0) { dbP.GetPrimaryKeys(); }

                        foreach (ColumnProperties cp in fkCollection)
                        {
                            StringBuilder selects = new StringBuilder();
                            string sSel = string.Empty;
                            string sParams = string.Empty;
                            ...
                            sSel += nl + tab + "SELECT";
                            sSel += nl + sSelList;
                            fkFieldName = cp.Column.ToUpper();
                            sWhere += nl + tab + tab + "(@" + cp.Column + " = " + cp.Column + ")";
                            sParams += nl + tab + "@" + cp.Column + " " + cp.ColumnDefinition();
                            if (sWhere.Length > 0) { sWhere = nl + tab + "WHERE" + nl + tab + "(" + sWhere.Remove(sWhere.Length - 4, 4) + nl + tab + ")"; }
                            if (sParams.Length > 0) { sParams = nl + "(" + sParams.Remove(sParams.Length - 2, 2) + nl + ")"; }
                            sSel += nl + tab + "FROM [" + dbP.Schema + "].[" + ((dbP.Table.Length > 0) ? dbP.Table : dbP.View) + "]";
                            sSel += sWhere;
                            sSel += nl + "END";
                            selects.AppendLine("CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_SelectBy" + fkFieldName + "]");
                            selects.AppendLine(sParams);
                            selects.Append(sSel);
                            selects.AppendLine();
                            selCollection.Add(selects.ToString());
        ...
        private List<string> BuildDefaultInserts()
        {
            List<string> iCollection = new List<string>();
            StringBuilder inserts = new StringBuilder();
            string sSel = string.Empty;
            TreeNode treeNode = GetNode("nodeInsertsPK");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                sSel = "CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_Insert]";
                if (bLoading == false)
                {
                    int iInsertCount = 0;
                    string sParams = string.Empty;
                    ...
                    foreach (ColumnProperties cp in dbP.Fields)
                    {
                        if (cp != null)
                        {
                            if ((cp.Column != sIdentityColumn))
                            {
                                sParams += nl + tab + "@" + CP(cp.Column) + " " + cp.ColumnDefinition() + ",";
                                sFields += nl + tab + tab + "[" + cp.Column + "],";
                                sValues += nl + tab + tab + "@" + CP(cp.Column) + ",";
                                iInsertCount++;
                            }
                        }
                    }
                    ...
                    sSel += nl + tab + "SET NOCOUNT ON;" + nl;
                    sSel += nl + tab + "DECLARE @RES " + dbP.GetIdentityType();
                ...
                inserts.AppendLine(sSel);
                iCollection.Add(inserts.ToString());
            }
            inserts.Clear();
            treeNode = GetNode("nodeInsertsIdentity");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                //inserts.AppendLine(" ; ");
                //inserts.AppendLine(" GO ");
                //inserts.AppendLine(" SET ANSI_NULLS ON " + nl + " GO " + nl + " SET QUOTED_IDENTIFIER ON " + nl + " GO ");
                if (bLoading == false)
                {
                    int iInsertCount = 0;
                    string sParams = string.Empty;
                    ...
                    string sFields = string.Empty;
                    string sIdentityColumn = dbP.GetIdentityColumn();
                    sSel = "CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_InsertIdentity]";
                    foreach (ColumnProperties cp in dbP.Fields)
                    {
                        if (cp != null)
                        {
                    ...
                    sSel += nl + tab + "SET NOCOUNT ON;" + nl;
                    sSel += nl + tab + "DECLARE @RES " + dbP.GetIdentityType();
                    sSel += nl + tab + "SET IDENTITY_INSERT " + dbP.Table + " ON;";
                    sSel += nl + tab + "INSERT INTO [" + dbP.Schema + "].[" + ((dbP.Table.Length > 0) ? dbP.Table : dbP.View) + "]";
                    sSel += nl + tab + "(" + sFields + nl + tab + ")";
                    sSel += nl + tab + "VALUES";
                    sSel += nl + tab + "(" + sValues + nl + tab + ")";
        ...
        private string BuildDefaultUpdates()
        {
            StringBuilder updates = new StringBuilder();
            TreeNode treeNode = GetNode("nodeUpdates");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                string sSel = "CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_Update]";
                if (bLoading == false)
                {
                    int iWhereCount = 0;
                    int iSelectCount = 0;
                    ...
                    foreach (ColumnProperties cp in dbP.Fields)
                    {
                        if (hshParam[cp.Column] == null) { hshParam.Add(cp.Column, cp.Column); }
                        if (cp.PriKey)
                        {
                            //sParams += nl + tab + "@" + sItem + " " + ((ColumnProperties)dbP.Columns[sItem]).ColumnDefinition() + ",";
                            iWhereCount++;
                            sWhere += nl + tab + tab + "([" + cp.Column + "] = @" + CP(cp.Column) + ") AND";
                        }
                        else
                        {
                            sUpdate += nl + tab + tab + "[" + cp.Column + "]=@" + CP(cp.Column) + ",";
                            iSelectCount++;
                        }
                    }
                    //generte Paramslist
                ...
                updates.AppendLine(sSel);
                //updates.AppendLine(" ; ");
                //updates.AppendLine(" GO ");
                //updates.AppendLine(" SET ANSI_NULLS ON " + nl + " GO " + nl + " SET QUOTED_IDENTIFIER ON " + nl + " GO ");
            }
            return updates.ToString();
        }
        
        private string BuildDefaultDeletes()
        {
            StringBuilder deletes = new StringBuilder();
            TreeNode treeNode = GetNode("nodeDeletes");
            if (treeNode != null && treeDefaultSPOptions.GetChecked(treeNode) == UIControls.TreeViews.TriStateTreeView.CheckState.Checked)
            {
                if (bLoading == false)
                {
                    ...
                    string sParams = string.Empty;
                    string sWhere = nl + tab + "WHERE" + nl + tab + "(";
                    string spkfieldName = string.Empty;
                    foreach (ColumnProperties cp in dbP.Fields)
                    {
                        if (!string.IsNullOrEmpty(cp.Column))
                        {
                            if (cp.PriKey)
                            {
                                spkfieldName = cp.Column;
                                sParams += nl + tab + "@" + CP(cp.Column) + " " + cp.ColumnDefinition() + ",";
                                iWhereCount++;
                                sWhere += nl + tab + tab + "([" + cp.Column + "] = @" + CP(cp.Column) + ") AND";
                            }
                    ...
                    string sSel = "CREATE PROCEDURE [" + dbP.Schema + "].[" + dbP.Table + "_DeleteBy" + spkfieldName.ToUpper() + "]";
                    if (iWhereCount > 0) { sSel += nl + "("; }
                    if (sWhere.Length > 20) { sWhere = sWhere.Substring(0, sWhere.Length - 3); } else { sWhere = string.Empty; }  //remove that trailing AND
                    if (sWhere.Length > 0) { sWhere += nl + tab + ")"; } else { sWhere = string.Empty; }
                    if (sParams.Length > 0) { sParams = sParams.Substring(0, sParams.Length - 1); } else { sParams = string.Empty; }     //remove the trailing comma
                    ...
                    deletes.AppendLine(sSel);
                    //deletes.AppendLine(" ; ");
                    //deletes.AppendLine(" GO ");
                    //deletes.AppendLine(" SET ANSI_NULLS ON " + nl + " GO " + nl + " SET QUOTED_IDENTIFIER ON " + nl + " GO ");
                }
            }
            return deletes.ToString();
        }
		...
		private string BuildSelectSP()
		{
			...
			if (bLoading == false)
			{
				...
				sSel += sOrderBy;
				sSel += nl + "END";
                //check if the selectWhereList contains an item that is not part of an index or foreign key
                bool displayIndexWarning = false;
                string warningColumns = string.Empty;
                foreach (whereItem item in selectWhereList)
                {
                    ColumnProperties col = dbP.Fields.Find( delegate(ColumnProperties column){return column.Column == item.ColName;} );
                    bool hasIndex = false;
                    foreach(KeyValuePair<string,dbIndex> kvp in dbP.Indices)
                    {
                        if(kvp.Value.IndexColumns.Contains(col.Column)) { hasIndex=true;}
                    }
                    if(col!=null && !col.PriKey && !col.ForeignKeyProperties.IsFK && !hasIndex)
                    {
                        warningColumns += col.Column + ", ";
                        displayIndexWarning = true;
                    }
                }
                if(!string.IsNullOrEmpty(warningColumns)) { warningColumns = warningColumns.Substring(0,warningColumns.Length-2);}
                if (displayIndexWarning) { DisplayMsgDetails("Performance Tip! Consider adding an Index for the '" + warningColumns + "' column(s)", 3); }
			}
			return sSel;
		}
		...
		public void GenerateCode(String ObjName, ref System.Windows.Forms.RichTextBox tb)
		{
			...
			else if (this.rdoTiersSingle.Checked)//Single Result Tiers Code
			{
                ...
                {
                    dataclassName = dbTableParameters.StripSchema(ObjName);
                ...
                if (string.IsNullOrEmpty(dataaccessclassName))
                {
                    dataaccessclassName = dbTableParameters.StripSchema(CP(ObjName));
                    ...
                    if (newdataaccessclassName != null)
                    {
				...
				for (int i = 0; i < _tableCount; i++)
				{
					if (this.rdoCSharp.Checked)
					{
						sbInterface.AppendLine("public interface I" + dataclassName + "_" + i.ToString());
						sbInterface.AppendLine("{");
						sbDTO.AppendLine("public class " + dataclassName + "_" + i.ToString()+ ": " + "I" + dataclassName + "_" + i.ToString() + ", INotifyPropertyChanged");
						sbDTO.AppendLine("{");
					...
					{
						sbInterface.AppendLine("Public Interface I" + dataclassName + "_" + i.ToString());
						sbDTO.AppendLine("Public Class " + dataclassName + "_" + i.ToString() + " Implements I" + dataclassName + "_" + i.ToString() + ", INotifyPropertyChanged");
				...
				if (ValidConnection() && cboSP.Text != "")
				{
					if (this.rdoCSharp.Checked)
					{
						...
						for(int i=0;i<_tableCount;i++)
						{
							_variablePrefix += tab + tab + dataclassName + "_" + i.ToString() + " _" + dataclassName + "_" + i.ToString() + " = new " + dataclassName + "_" + i.ToString() + "();" + nl;
							_exeParms += "out I" + dataclassName + "_" + i.ToString() + " " + (dataclassName + "_" + i.ToString()).ToLower() + ", ";
						}
						...
						for (int _currentResultSet = 0; _currentResultSet < _tableCount; _currentResultSet++)
						{
							...
							if (returnColumns.Count > 0)
							{
								...
								foreach (ColumnProperties cp in returnColumns)
								{
									...
									{
										_code += tab + tab + tab + tab + tab + tab + tab + tab + cp.Cast2NetClassCode("_" + dataclassName + "_" + cp.TableOrdinal.ToString()) + tab + tab + "// return field " + cp.Column + " [" + cp.Type + "]" + nl;
									}
								}
							}
							else
						...
						for (int i = 0; i < _tableCount; i++)
						{
							_return += tab + tab +( dataclassName + "_" + i.ToString()).ToLower() + " = (I" + dataclassName + "_" + i.ToString() + ")_" + dataclassName + "_" + i.ToString() + ";" + nl;
						}
					...
					else
					{
						...
						for (int i = 0; i < _tableCount; i++)
						{
							_variablePrefix += tab + tab + "Dim _" + dataclassName + "_" + i.ToString() + " As " + dataclassName + "_" + i.ToString() + " = New " + dataclassName + "_" + i.ToString() + "()" + nl;
							_exeParams += "ByRef " + (dataclassName + "_" + i.ToString() + " ").ToLower() + " As I" + (dataclassName + "_" + i.ToString()) + ", ";
						}
						...
						for (int _currentResultSet = 0; _currentResultSet < _tableCount; _currentResultSet++)
						{
							...
							if (returnColumns.Count > 0)
							{
								...
								foreach (ColumnProperties cp in returnColumns)
								{
									if (cp.TableOrdinal == _currentResultSet)
									{
										_code += tab + tab + tab + tab + tab + tab + tab + tab + cp.Cast2NetClassCode("_" + dataclassName + "_" + cp.TableOrdinal.ToString()).Replace(";", "") + tab + tab + "' return field " + cp.Column + " [" + cp.Type + "]" + nl;
									}
								}
							}
							else
						...
						for (int i = 0; i < _tableCount; i++)
						{
							_return += tab + tab +( dataclassName + "_" + i.ToString()).ToLower() + " = CType(_" + dataclassName + "_" + i.ToString() + ", I" + dataclassName + "_" + i.ToString() + ")" + nl;
						}
						_return += tab + tab + "Return RETURN_VALUE" + nl;
			...
			else if (this.rdoTiersMultiple.Checked)//Collection Result Tiers Code
			{
				...
				if (ValidConnection() && cboSP.Text != "")
				{
					if (this.rdoCSharp.Checked)
					{
						...
						for (int i = 0; i < _tableCount; i++)
						{
							_variablePrefix += tab + tab + "System.Collections.Generic.List<I" + dataclassName + "_" + i.ToString() +"> _" + dataclassName + "_" + i.ToString() + " = new System.Collections.Generic.List<I" + dataclassName + "_" + i.ToString() + ">();" + nl;
							_exeParms += "out System.Collections.Generic.List<I" + dataclassName + "_" + i.ToString() + "> " + (dataclassName + "_" + i.ToString()).ToLower() + ", ";
						}
						...
						for (int _currentResultSet = 0; _currentResultSet < _tableCount; _currentResultSet++)
						{
							...
							if (returnColumns.Count > 0)
							{
								_code += tab + tab + tab + tab + tab + tab + tab + tab + "//read your results here" + nl;
								_code += tab + tab + tab + tab + tab + tab + tab + tab + "//review the text of the stored procedure for any additional results that may be returned" + nl;
								_code += tab + tab + tab + tab + tab + tab + tab + tab + (dataclassName) + "_" + _currentResultSet.ToString() + " l_" + (dataclassName) + "_" + _currentResultSet.ToString() + " = new " + (dataclassName) + "_" + _currentResultSet.ToString() + "();" + nl;
								foreach (ColumnProperties cp in returnColumns)
								{
									if (cp.TableOrdinal == _currentResultSet)
									{
										_code += tab + tab + tab + tab + tab + tab + tab + tab + cp.Cast2NetClassCode("l_" + dataclassName + "_" + cp.TableOrdinal.ToString()) + tab + tab + "// return field " + cp.Column + " [" + cp.Type + "]" + nl;
									}
								}
                                _code += tab + tab + tab + tab + tab + tab + tab + tab + "_" + dataclassName + "_" + _currentResultSet.ToString() + ".Add(l_" + (dataclassName) + "_" + _currentResultSet.ToString() + ");" + nl;
							}
							else
							{
								_code += tab + tab + tab + tab + tab + tab + "// SAMPLE: string _variable = rdr[\"Name\"].ToString();" + nl;
						...
						for (int i = 0; i < _tableCount; i++)
						{
							_return += tab + tab + (dataclassName + "_" + i.ToString()).ToLower() + " = _" + dataclassName + "_" + i.ToString() + ";" + nl;
						}
						_return += tab + tab + "return RETURN_VALUE;" + nl;
						CodeHighlight(sbInterface.ToString() + nl + sbDTO.ToString() + nl + _method + _variablePrefix + _try + _code + tab + tab + "}" + nl + _catch + _return + nl + tab + "}" + nl + "}" + nl);
					}
					else
					{
						...
						for (int i = 0; i < _tableCount; i++)
						{
							_variablePrefix += tab + tab + "Dim _" + dataclassName + "_" + i.ToString() + " As System.Collections.Generic.List(Of I" + dataclassName + "_" + i.ToString() + ") = New System.Collections.Generic.List(Of I" + dataclassName + "_" + i.ToString() + ")()" + nl;
							_exeParams += "ByRef " + (dataclassName + "_" + i.ToString() + " ").ToLower() + " As System.Collections.Generic.List(Of I" + (dataclassName + "_" + i.ToString()) + "), ";
						}
						if (_exeParams.Length > 0) { _exeParams = _exeParams.Substring(0, _exeParams.Length - 2); }
						_method += _exeParams + ") As Integer" + nl;
						...
						for (int _currentResultSet = 0; _currentResultSet < _tableCount; _currentResultSet++)
						{
							...
							if (returnColumns.Count > 0)
							{
								_code += tab + tab + tab + tab + tab + tab + tab + tab + "'read your results here" + nl;
								_code += tab + tab + tab + tab + tab + tab + tab + tab + "'review the text of the stored procedure for results that may be returned" + nl;
								_code += tab + tab + tab + tab + tab + tab + tab + tab + "Dim l_" + dataclassName + "_" + _currentResultSet.ToString() + " As " + dataclassName + "_" + _currentResultSet.ToString() + " = New " + dataclassName + "_" + _currentResultSet.ToString() + "()";
								foreach (ColumnProperties cp in returnColumns)
								{
									if (cp.TableOrdinal == _currentResultSet)
									{
										_code += tab + tab + tab + tab + tab + tab + tab + tab + cp.Cast2NetClassCode("l_" + dataclassName + "_" + cp.TableOrdinal.ToString()).Replace(";", "") + tab + tab + "' return field " + cp.Column + " [" + cp.Type + "]" + nl;
									}
								}
								_code += tab + tab + tab + tab + tab + tab + tab + tab + "_" + dataclassName + "_" + _currentResultSet.ToString() + ".Add(l_" + dataclassName + "_" + _currentResultSet.ToString() + ")" + nl;
							}
        ...
        private void cmdDefaultSPGenerate_Click(object sender, EventArgs e)
        {
            this.rtbDefaultSPs.Rtf = sh.Highlight(BuildDefaultProcedures());
        }
